...
const { isNonEmptyString, isFiniteNumber } = require('../validation');
...
function getAll() { return readAll().sort((a, b) => a.from.localeCompare(b.from)); }
function getRateForMonth(year, month) {
  const key = `${year}-${String(month).padStart(2, '0')}`;
  const sorted = getAll();
  if (!sorted.length) return 0;
  let applicable = sorted[0].rate;
  for (const entry of sorted) { if (entry.from <= key) applicable = entry.rate; else break; }
  return applicable;
}
function validate({ from, rate }) {
  if (!isNonEmptyString(from) || !/^\d{4}-\d{2}$/.test(from)) throw new Error('Rate start month must be YYYY-MM');
  const month = Number(from.slice(5));
  if (month < 1 || month > 12) throw new Error('Rate start month must be valid');
  if (!isFiniteNumber(rate, { min: 0 })) throw new Error('Rate must be a non-negative number');
}
function add({ from, rate }) {
  validate({ from, rate });
  const list = readAll();
  const entry = { id: `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`, from, rate: Number(rate) };
  list.push(entry); fs.writeFileSync(DATA_FILE, JSON.stringify(list, null, 2)); return entry;
}
function update(id, { from, rate }) {
  validate({ from, rate });
  const list = readAll(); const idx = list.findIndex(r => r.id === id);
  if (idx === -1) throw new Error(`Rate not found: ${id}`);
  list[idx] = { ...list[idx], from, rate: Number(rate) };
  fs.writeFileSync(DATA_FILE, JSON.stringify(list, null, 2)); return list[idx];
}
function remove(id) {
  const list = readAll();
